    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context to automatically extract entities and relationships from the knowledge graph, initialize simulation individuals, and assign unique behaviors and memories based on real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, active time periods, posting frequency, and event triggers for each individual based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - only show after config is generated and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real-world time, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min of time elapsed</span>
              </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    if (!configTimer) {
      addLog('Starting Generate Dual-Platform Simulation Config...')
      startConfigPolling()
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Read ${res.data.expected_entities_count} agent entities from Zep graph`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          addLog(`  ├─ Agent count: ${data.summary.total_agents} agents`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config is being generated, starting to poll...')
        startConfigPolling()
      }
    }
  } catch (err) {
