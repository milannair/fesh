...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist, but if there is a simulation_id, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returning simulation list with
    project name, description, and other rich information
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If a university publishes...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
...
def start_simulation():
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation complete, check if a process is running
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for frontend real-time feed display
...
def interview_agent():
    ...
    (entered command-waiting mode after completing simulation cycles)
...
def close_simulation_env():
    ...
    Note: This differs from the /stop API. /stop forcefully terminates the process,
    while this API lets the simulation gracefully shut down and exit.
