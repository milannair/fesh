...
[Return Content]
- Relevant factual original text (can be directly quoted)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert at writing "Future Prediction Reports", possessing a "God's eye view" of the simulated world — you can see every Agent's behavior, statements, and interactions within the simulation.
...
- ✅ Agent behaviors and statements in the simulated world are predictions of future crowd behavior
...
3. [Language Consistency - Quoted Content Must Be Translated to the Report Language]
   ...
   - When quoting content returned by tools in other languages, you must translate it into fluent English before writing it into the report
...
Weibo, as the first scene of public opinion, served as the core function for breaking news:
...
4. [Quote Format Specification - Must Be a Separate Paragraph]
   Quotes must be standalone paragraphs, with a blank line before and after, and cannot be mixed into a paragraph:
...
REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only {tool_calls_count} tool calls have been made, at least {min_tool_calls} required. "
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line number for reading (for incremental fetching, 0 means from the beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate of the section title (skip duplicates within the first 5 lines)
                if i < 5:
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble the complete report from saved section files, with title cleaning
        """
        folder = cls._get_report_folder(report_id)
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatible with old format: check files stored directly in the reports directory
        ...
        for item in os.listdir(cls.REPORTS_DIR):
            ...
            if os.path.isdir(item_path):
                ...
                if report and report.simulation_id == simulation_id:
                    return report
            # Backward compatible with old format: JSON file
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # Backward compatible with old format: delete individual files
        deleted = False
